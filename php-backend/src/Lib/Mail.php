...
class Mail
{
    private static function getTransporter(): PHPMailer
    {
        $mail = new PHPMailer(true);
        $mail->isSMTP();
        // Since it's 'gmail' in JS, we use smtp.gmail.com
        ...
        $mail->setFrom($_ENV['SMTP_USER'] ?? 'support@swastikfashion.com', 'Swastik Fashion');
        return $mail;
    }

    private static function buildWelcomeEmailHtml(string $name, string $email): string
    {
        $firstName = explode(' ', $name)[0] ?: $name;
        // Keep it simple and use string interpolation, we can use output buffering or heredoc
        $html = <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
...
HTML;
    ...
    public static function sendWelcomeEmail(string $toEmail, string $businessName): void
    {
        try {
            $mail = self::getTransporter();
            $mail->addAddress($toEmail);
            $firstName = explode(' ', $businessName)[0] ?: $businessName;
            $mail->Subject = "Welcome to Swastik Fashion, {$firstName}! 🎉";
            $mail->isHTML(true);
            $mail->Body = self::buildWelcomeEmailHtml($businessName, $toEmail);
            
            $mail->send();
        } catch (\Exception $e) {
            // Silently fail as in JS
        }
    }
    ...
    private static function parseUserAgent(string $ua = ''): array
    {
        ...
        elseif (str_contains($ua, 'iPad') || (str_contains($ua, 'Android') && !str_contains($ua, 'Mobile'))) $device = 'Tablet';

        return ['browser' => $browser, 'os' => $os, 'device' => $device];
    }

    private static function buildLoginAlertHtml(array $data): string
    {
        $name = $data['name'] ?? $data['email'];
        $email = $data['email'];
        $firstName = explode(' ', $name)[0];
        $parsed = self::parseUserAgent($data['userAgent'] ?? '');
        ...
        $dt = new \DateTime();
        $dt->setTimestamp($loginTime);
        $dt->setTimezone(new \DateTimeZone('Asia/Kolkata'));
        $timeStr = $dt->format('l, F j, Y h:i:s A') . ' IST';
        
        $deviceIcon = $device === 'Mobile' ? '📱' : ($device === 'Tablet' ? '📟' : '💻');
        $methodLabel = ($data['method'] ?? '') === 'google' ? 'Google Sign-In' : 'Email & Password';
        $methodIcon = ($data['method'] ?? '') === 'google' ? '🔵' : '🔐';
  ...
  <div class="body">
    ...
    <div class="info-box">
      ...
      <div class="info-rows">
        ...
        <div class="info-row"><span class="info-k">🌐 &nbsp;IP Address</span><span class="info-v">$ip</span></div>
...
HTML;
    ...
    public static function sendLoginAlertEmail(string $toEmail, string $name, array $opts = []): void
    {
        try {
            $mail = self::getTransporter();
            $mail->setFrom($_ENV['SMTP_USER'] ?? '', 'Swastik Fashion Security');
            $mail->addAddress($toEmail);
            $mail->Subject = "🔔 New sign-in to your Swastik Fashion account";
            $mail->isHTML(true);
            $opts['email'] = $toEmail;
            $opts['name'] = $name;
            $mail->Body = self::buildLoginAlertHtml($opts);
            $mail->send();
        } catch (\Exception $e) {
            // Silently fail
        }
